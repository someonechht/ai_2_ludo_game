...
int main()
{
    ...
    //         agent_trained++;
    //         // test_agent(*agents[i].get(),100, agents_score[i], std::ref(job_finished[available_thread]));
    //         // std::cout << "\nprogress: " << agent_trained << " / " << agents.size();
    //     }
    ...
    //     // copy copy higest score agent to best agent
    ...
    //         << " worst: " << worst 
    //         << std::endl;
    //     // generate new agent based on previous best agent
    //     for (auto& agent : agents)
    //     {
    //         // copy previous best agents brain over
    ...
    //     // wwrite best agents weight to file
    ...
    std::array<std::string, 5> best_nn_paths = {
        "best_nn_var_1.txt", 
        "best_nn_var_2.txt", 
        "best_nn_var_3.txt", 
        "best_nn_var_4.txt", 
    ...
    std::array<std::string, 5> overall_best_nn_paths = {
        "overall_best_var_1.txt", 
        "overall_best_var_2.txt", 
        "overall_best_var_3.txt", 
        "overall_best_var_4.txt", 
        "overall_best_var_5.txt" 
        };

    int games = 10000;
    std::vector<std::thread> threads;

    std::cout << "evaluating best nn" << std::endl;
    for (int i = 0; i < 5; i++)
        threads.emplace_back(std::thread(test_brain, best_nn_paths[i], games));
    for (auto& th : threads) 
        th.join();

    threads.clear();
    std::cout << "evaluating overall best nn" << std::endl;
    for (int i = 0; i < 5; i++)
        threads.emplace_back(std::thread(test_brain, overall_best_nn_paths[i], games));
    for (auto& th : threads) 
        th.join();
